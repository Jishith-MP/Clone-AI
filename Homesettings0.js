...
import { getAuth, deleteUser } from 'https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js';
import { getFirestore, collection, deleteDoc, getDocs } from 'https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js';
// Initialize Firebase
...
// Function to clear user chat history
async function clearUserHistory(uid) {
  try {
    const historyRef = collection(db, `userChatHistory/${uid}/history`);
  ...
  try {
   ...
   window.location.href = 'Login.html';
    ...
    console.error("Error deleting user:", error);
      window.alert("User not authenticated. Please log in again and delete Account permenantly.");
  }
}

// Event listener for clearing chat history
document.getElementById('clear').addEventListener('click', () => {
  let b = document.getElementById('delete-chathis-box');
  b.style.display = 'block';
});

// Event listener for canceling chat history deletion
document.getElementById('cancelButtn').addEventListener('click', () => {
  let b = document.getElementById('delete-chathis-box');
...
document.getElementById('delete-BTN').addEventListener('click', () => {
  if (uid) {
    ...
    // Remove saved response from session storage
    sessionStorage.removeItem('saveResponse');

    // Alert user and reload page
    window.alert('Chat history deleted successfully!');
    setTimeout(() => {
...
// Event listener for confirming account deletion
document.getElementById('Acdlt').addEventListener('click', () => {
  let A = document.getElementById('delete-confirm-box');
...
// Event listener for canceling account deletion
