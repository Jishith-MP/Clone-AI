...
const policiesRef = collection(db, 'mainpagearticle');
...
function displayPolicyData(policyData) {
  const policyContainer = document.getElementById('article');
  ...
  headingElement.textContent = policyData.heading || ''; // Ensure heading is defined
    headingElement.classList.add('headingdb');
  policyContainer.appendChild(headingElement);

  // Add content if available
  if (policyData.content || policyData.secondcontent) {
    const contentsList = document.createElement('p');
    contentsList.classList.add('articlep'); // Add class name to the <ul>
    
    if (policyData.content) {
      const listItem = document.createElement('p');
      listItem.textContent = policyData.content;
      listItem.classList.add('articlep'); // Add class name to the <li>
      contentsList.appendChild(listItem);
    }
    
    if (policyData.secondcontent) {
      const secondListItem = document.createElement('b');
      secondListItem.textContent = policyData.secondcontent;
      secondListItem.classList.add('articled'); // Add class name to the <li>
