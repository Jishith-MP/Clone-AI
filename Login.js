...
const signInuser = evt => {
  ...
  if (!agreeCheckbox.checked) {
        var policiesandcon = document.getElementById('policies');
        policiesandcon.innerHTML = `<p>accept our policies and conditions to continue Clone AI.Read our<a href="privacy policy.html" style="color: blue;">privacy & policy</a> and <a href="privacy policy.html" style="color: hotpink;">terms & conditions</a></p>`;
             policiesandcon.style.color = 'red';
        return; // Prevent further execution
  }

  // Proceed with authentication if the checkbox is checked
  ...
  signInWithEmailAndPassword(auth, EmailInp.value, PassInp.value)
    .then((credentials) => {
      ...
      get(userRef).then((snapshot) => {
        if (snapshot.exists()) {
          localStorage.setItem("user-info", JSON.stringify({
            ...
            lastname: snapshot.val().secondname
          }));
      ...
      .finally(() => {
        // Hide spinner after redirection or error
    ...
    .catch((error) => {
      alert("Email ID or password does not match our records. Please check the email/password to access CLONE AI.");
      console.error("Authentication error:", error);
      // Hide spinner in case of authentication error
      document.getElementById("spinner").style.display = "none";
      document.getElementById("overlay").style.display = "none";
    });
};
